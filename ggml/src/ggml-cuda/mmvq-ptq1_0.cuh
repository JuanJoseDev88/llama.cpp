...
// The PT layout is not NVIDIA-specific: the trit decode is integer arithmetic and
// the dot products go through ggml_cuda_dp4a and the byte permute, which map onto
// v_sdot4/v_sudot4 and v_perm on every AMD target that runs PTQ1_0 at all. The
// quantizer and the mat-vec dispatch both call this, so they cannot disagree.
...
// This is the only selector the decode below uses. v_perm_b32/amdgcn_perm pick one
// whole byte per output position: they read 0x7531 as bytes {0x31,0x75,0,0} and
// return the bogus byte0||byte0||ff||ff pattern, constant selector or not.
// __byte_perm reads nibbles {1,3,5,7} and differs. Emit its ALU equivalent instead.
static __device__ __forceinline__ int ptq1_0_pt_byte_perm(const uint32_t a, const uint32_t b, const uint32_t sel) {
...
static __device__ __forceinline__ int ptq1_0_trit_step(uint32_t & vlo, uint32_t & vhi) {
    ...
    vlo = wlo & 0x00FF00FF;
    vhi = whi & 0x00FF00FF;
    // byte_perm(wlo, whi, 0x7531): interleave the high bytes of the 16-bit lanes.
    // v_perm_b32 takes one whole byte per output position, so the nibble selector
    // 0x7531 = {1,3,5,7} becomes the byte selector 0x03010705 (see ptq1_0_pt_byte_perm).
#if defined(__HIP_DEVICE_COMPILE__)
    const uint32_t sel = 0x03010705u;
    uint32_t q;
    asm("v_perm_b32 %0, %1, %2, %3" : "=v"(q) : "v"(wlo), "v"(whi), "s"(sel));
    return ptq1_0_trits_to_weights((int) q);
#else
    const uint32_t q = (uint32_t) __byte_perm(wlo, whi, 0x7531);
    return ptq1_0_trits_to_weights((int) q);
#endif
}
...
#if defined(__HIP_DEVICE_COMPILE__)
    const uint32_t sel_lo = 0x00050004u, sel_hi = 0x00070006u;
#endif
#pragma unroll
    for (int i = 0; i < nrows; ++i) {
#pragma unroll
        for (int g = 0; g < 4; ++g) {
            ...
            // widen: spread bytes {0,1}/{2,3} into the low halves of the two 16-bit lanes.
            // One permute per half; byte selectors {4,0,5,0} and {6,0,7,0} pick
            // {p.b0,0,p.b1,0} and {p.b2,0,p.b3,0} (v_perm indexes whole bytes).
#if defined(__HIP_DEVICE_COMPILE__)
            uint32_t lo, hi;
            asm("v_perm_b32 %0, %1, 0, %2" : "=v"(lo) : "v"(packed), "s"(sel_lo));
            asm("v_perm_b32 %0, %1, 0, %2" : "=v"(hi) : "v"(packed), "s"(sel_hi));
            vlo[i][g] = lo;
            vhi[i][g] = hi;
#else
            vlo[i][g] = (packed & 0xFFu) | ((packed & 0xFF00u) << 8);
            vhi[i][g] = ((packed >> 16) & 0xFFu) | (((packed >> 24) & 0xFFu) << 16);
#endif
        }
    }
#pragma unroll
...
#pragma unroll
        for (int g = 0; g < 2; ++g) {
            ...
            vlo2[i][g] = (packed & 0xFFu) | ((packed & 0xFF00u) << 8);
            vhi2[i][g] = ((packed >> 16) & 0xFFu) | (((packed >> 24) & 0xFFu) << 16);
        }
    }
    int4 u2[ncols];
#pragma unroll
...
#pragma unroll
        for (int t = 0; t < 4; t += 2) {
            ...
            v                 = w1 & 0x00FF00FF;
            const int q = ptq1_0_trits_to_weights(ptq1_0_pt_byte_perm(w0, w1, 0x7531));
#pragma unroll
            for (int j = 0; j < ncols; ++j) {
...
static __host__ int ptq1_0_pt_rows_per_cta(const int blocks_per_row, const int ncols_dst, const int nrows_x, const int rows_per_item) {
    ...
    for (int r = rows_per_item; r <= rmax; r += rows_per_item) {
        ...
        if (util + 1e-9 >= best_util) { // tie-break to the larger r: more items per thread lets the loop pipeline loads across items
            best_util = util;
            best = r;
        }
        if (util > 0.999) {
    ...
    while (best > rows_per_item && (nrows_x + best - 1) / best < 160) { // keep >=160 CTAs so small tensors still fill the GPU
        best -= rows_per_item;
    }
    return best;
}
...
#pragma unroll
        ...
        asm volatile("" ::: "memory"); // keep the gate loads after the main stores: without it LLVM overlaps both block_dot live ranges (207 VGPR, 4 waves)
...
static void mul_mat_vec_ptq1_0_pt_launch(
        ...
        const int ncols_x, const int nrows_x, const int stride_row_x, const int stride_col_y, const int stride_col_dst,
        cudaStream_t stream) {
    constexpr int ROWS = 1; // one row per item keeps the trit decode state tiny: 83 VGPR -> 11 resident waves/SIMD
