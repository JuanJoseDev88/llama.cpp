...
bool ggml_cuda_should_use_mmq(enum ggml_type type, int cc, int64_t ne11, int64_t n_experts) {
    ...
    switch (type) {
        case GGML_TYPE_PTQ1_0:
            // the tile kernels are plain integer SIMT code: dp4a (v_sdot4/v_sudot4) on
            // pre-RDNA3 AMD and WMMA/MFMA elsewhere, no NVIDIA-specific instructions
            mmq_supported = true;
            break;
        case GGML_TYPE_Q1_0:
        case GGML_TYPE_Q2_0:
...
// -------------------------------------------------
    ...
    if (type == GGML_TYPE_PTQ1_0) {
        // the fp16 dequantize + hipBLAS fallback is the source of PTQ1_0's extra error on
        // both NVIDIA and AMD, so the MMQ tile path runs at every batch by default; the env
        // var is the A/B knob for deployments that prefer hipBLAS's ~7% at pp512 over the
        // accuracy
